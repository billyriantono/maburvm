    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">
          Images captured from your VMs — they survive VM deletion and can seed a new VM
        </p>
      </div>

      {error && (
        <div className="rounded-lg border border-destructive/50 bg-destructive/10 p-6 shadow-sm">
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border bg-card p-12 shadow-sm text-center">
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Capture one from a VM&apos;s page (Save as Image).
          </p>
        </div>
      ) : (
              ...
              <div className="col-span-2 flex justify-end items-center gap-1">
                {image.status === "available" && (
                  <Button asChild variant="outline" size="sm" className="gap-1">
                    <Link href={`/client/order?source_image_id=${image.id}`}>
